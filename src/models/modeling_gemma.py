...
class GemmaInfiniAttention(GemmaAttention):
    def __init__(
        ...
        self.gate = nn.Parameter(torch.full((1, self.num_heads, 1, 1), 0.))
    ...
    def forward(
        ...
        **kwargs,
        ...
        causal_mask = attention_mask
        if attention_mask is not None:
            causal_mask = causal_mask[:, :, :, :self.segment_size]  # FIXME: This is wrong, should be [:, :, :, :self.segment_size]
    ...
    def _retrieve_from_memory(self, query_states, memory, norm_term):
        ...
        memory_output = torch.matmul(query_states, memory.repeat(1, self.num_key_value_groups, 1, 1))
    ...
    def _update_memory(self, key_states, value_states, memory, norm_term):
        ...
        # value_states = F.normalize(value_states, p=2, dim=-1)
...
class GemmaPreTrainedModel(PreTrainedModel):
    ...
    def _init_weights(self, module):
        ...
        if isinstance(module, nn.Linear):
            ...
            torch.nn.init.constant_(module.data, -80)
            # if module.padding_idx is not None:
            #     module.weight.data[module.padding_idx].zero_()
...
class GemmaForCausalLM(GemmaPreTrainedModel):
    ...
    def __init__(self, config):
        ...
        # self.model = convert_linear_layer_to_lora(self.model, 'layers.',  64)
        self.vocab_size = config.vocab_size
