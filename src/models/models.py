...
class ChatglmModel:
    def __init__(self, plm = 'llms/chatglm2-6b') -> None:

        self.tokenizer = AutoTokenizer.from_pretrained(plm, trust_remote_code=True)
        self.model = ChatGLMForConditionalGeneration.from_pretrained(plm, trust_remote_code=True).half().cuda()
...
class LLama:
    ...
    def generate(self, text, temperature=0.7,
                 ...
                 top_p=0.8, max_new_tokens=256, memory=None, norm_term=None, no_memory_update=False):
        ...
        outputs, memory, norm_term = self.model.generate(**inputs, do_sample=True, temperature=temperature, top_p=top_p,
                                      max_length=max_new_tokens + inputs['input_ids'].size(-1), memory=memory, norm_term=norm_term, no_memory_update=no_memory_update)

        response = self.tokenizer.decode(outputs[0][inputs.input_ids.shape[1]:], skip_special_tokens=True)
        return response, memory, norm_term
