...
import json
import random
from process_datasets import process_instruction, read_test, calculate_precision_recall_f1, calculate_precision_recall_f1_strict, read_test_label, post_process, process_instruction_only_rams, process_instruction_only_rams_wo_label
...
class EAE_train_dataset(torch.utils.data.Dataset):
    def __init__(self, data_dir, dataset, tokenizer, max_length, rag=False):
        ...
        self.data = process_instruction_only_rams_wo_label(data_dir, dataset, rag)
        self.tokenizer = tokenizer
...
if __name__ == '__main__':
    ...
    train(args)           
'''
 nohup accelerate launch --config_file /223040263/wanlong/LLM_Retreival/RAG4RE/src/train/configs/sft_zero1.yaml --num_processes 4 --num_machines 1 --machine_rank 0  --deepspeed_multinode_launcher  standard  /223040263/wanlong/LLM_Retreival/RAG4RE/src/train/train_llm_EE.py    --experiment_name EAE_train_no_retrieval   --model_path /sds_wangby/models/Meta-Llama-3-8B-Instruct/     --max_ckpts 3      --max_seq_len 4800    --gradient_accumulation_steps 16     --output_dir ./train_EE_v1    --log_dir ./train_EE     --n_epochs 3    --train_bsz_per_gpu 3     --eval_bsz_per_gpu 3     --learning_rate 2e-5     --eval_step 100000    --save_step 100000    --gradient_checkpointing    > train_EE.log 2>&1 &
